...
def process_parameter(x, row_major, comp_bound):
    ...
    util_sys_arr = x[0]/(math.ceil(round(x[0]/A, 1))*A) \
                        * x[1]*x[2]/(math.ceil(round(x[1]*x[2]/A, 1))*A)
    ...
    res.append([int(total_transfer), int(total_cycle), util_sys_arr, \
                util_buf, x, Co/c_0, W/w_0, H/h_0, bound])
    return
...
def row_major_mem_obj(x):
    return (x[0]*x[1]*x[2] + (S*x[1]+2)*(S*x[2]+2)*Ci)*(H*W*Co/(x[0]*x[1]*x[2])-Co/x[0]) \
            + (K_h*K_w*Ci)*Co/x[0] + x[1]*x[2]*Co
...
def row_major_comp_bound_constraint(x):
    return K_h*K_w*Ci/(A*A)*x[0]*x[1]*x[2] \
...
def channel_major_mem_obj(x):
    return  (K_h*K_w*Ci*Co)/(x[1]*x[2])+2*(S*x[1]+S*x[2])*Co/(x[1]*x[2])+1/x[0]
